"""
Centralized logging setup for Tap.
...
Level is controlled by the TAP_LOG_LEVEL env var (falls back to
INFO if unset or invalid), so you can run in debug mode without touching
code:

    TAP_LOG_LEVEL=DEBUG python main.py
...
def configure_logging(level: str | None = None) -> None:
    ...
    Priority: explicit `level` arg > TAP_LOG_LEVEL env var > INFO.
    """
